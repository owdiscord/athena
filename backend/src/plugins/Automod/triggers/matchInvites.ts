...
export const MatchInvitesTrigger = automodTrigger<MatchResultType>()({
  ...
  async match({ pluginData, context, triggerConfig: trigger }) {
    ...
    for await (const [type, str] of matchMultipleTextTypesOnMessage(
      pluginData,
      trigger,
      context.message,
    )) {
      ...
      for (const code of uniqueInviteCodes) {
        if (
          trigger.include_invite_codes &&
          trigger.include_invite_codes.includes(code)
        ) {
          return { extra: { type, code } };
        }
        if (
          trigger.exclude_invite_codes &&
          !trigger.exclude_invite_codes.includes(code)
        ) {
          return { extra: { type, code } };
        }
        if (
          trigger.include_custom_invite_codes &&
          trigger.include_custom_invite_codes.includes(code.toLowerCase())
        ) {
          return { extra: { type, code } };
        }
        if (
          trigger.exclude_custom_invite_codes &&
          !trigger.exclude_custom_invite_codes.includes(code.toLowerCase())
        ) {
          return { extra: { type, code } };
        }
      }
      ...
      for (const code of uniqueInviteCodes) {
        ...
        if (
          trigger.include_guilds &&
          trigger.include_guilds.includes(invite.guild.id)
        ) {
          return { extra: { type, code, invite } };
        }
        if (
          trigger.exclude_guilds &&
          !trigger.exclude_guilds.includes(invite.guild.id)
        ) {
          return { extra: { type, code, invite } };
        }
      }
